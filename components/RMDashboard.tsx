...
export const RMDashboard: React.FC<RMDashboardProps> = ({ isOpen, onClose, data, okbRegionCounts, okbData, mode = 'modal', metrics, okbStatus, onEditClient, dateRange }) => {
    ...
    const prepareExportData = () => {
        ...
        const uncovered = okbData.filter(row => !row.lat || !activeCoords.has(`${row.lat.toFixed(4)},${row.lon.toFixed(4)}`));
        ...
        uncovered.forEach(row => {
            ...
            const r = findValueInRow(row, ['субъект', 'регион', 'область']) || 'Не указан';
            if (!hierarchy[c]) hierarchy[c] = new Set();
            hierarchy[c].add(r); countries.add(c); regions.add(r);
        });
    ...
    const performExport = () => {
        const rows = uncoveredRowsCache.filter(row => {
            const c = findValueInRow(row, ['страна', 'country']) || 'Россия';
            const r = findValueInRow(row, ['субъект', 'регион', 'область']) || 'Не указан';
            return selectedCountries.has(c) && selectedRegions.has(r);
        });
                            ...
                            return (
                                <React.Fragment key={rm.rmName}>
                                    <tr className={`hover:bg-gray-800/50 cursor-pointer transition-all ${isExp ? 'bg-gray-800/90' : ''}`} onClick={() => setExpandedRM(isExp ? null : rm.rmName)}>
                                        ...
                                        <td className="px-4 py-3 text-center font-mono text-white">{new Intl.NumberFormat('ru-RU').format(rm.totalFact)}</td>
                                        <td className="px-4 py-3 text-center font-mono text-gray-400">{rm.totalClients}/{rm.totalOkbCount || '?'}</td>
                                        <td className="px-4 py-3 text-center"><div className="flex flex-col items-center"><span className="text-xs font-bold text-indigo-300 mb-1">{rm.marketShare.toFixed(0)}%</span><div className="w-20 h-1 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${rm.marketShare}%` }}></div></div></div></td>
                                        ...
                                        <td className="px-4 py-3 text-center font-bold font-mono text-white bg-gray-800/20">{new Intl.NumberFormat('ru-RU').format(rm.nextYearPlan)}</td>
                                                            ...
                                                            <tbody className="divide-y divide-gray-700/50">
                                                                {rm.regions.map(reg => (
                                                                    <tr key={reg.name} className="hover:bg-indigo-500/20 cursor-pointer" onClick={() => handleRegionClick(rm.rmName, reg.name)}>
                                                                        ...
                                                                        <td className="px-3 py-2 text-right text-white font-medium">{new Intl.NumberFormat('ru-RU').format(reg.plan)}</td>
                                                                    </tr>
                                                                ))}
                                                            </tbody>
                                                        </table>
    ...
    return (
        <>
            ...
            {isRegionModalOpen && selectedRegionDetails && <RegionDetailsModal isOpen={isRegionModalOpen} onClose={() => setIsRegionModalOpen(false)} rmName={selectedRegionDetails.rmName} regionName={selectedRegionDetails.regionName} activeClients={selectedRegionDetails.activeClients} potentialClients={selectedRegionDetails.potentialClients} onEditClient={onEditClient} />}
            {isAbcModalOpen && <ClientsListModal isOpen={isAbcModalOpen} onClose={() => setIsAbcModalOpen(false)} title={abcModalTitle} clients={abcClients} onClientSelect={() => {}} onStartEdit={(c) => { onEditClient?.(c); setIsAbcModalOpen(false); }} showAbcLegend={true} />}
            ...
            <Modal 
                ...
                title="Настройка выгрузки непокрытых ТТ"
            ...
            >
                <div className="space-y-6">
                    <div className="p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-xl">
                        <p className="text-sm text-gray-200">Найдено <strong>{uncoveredRowsCache.length}</strong> ТТ в базе ОКБ, которые не сопоставлены с вашими активными продажами. Выберите страны для выгрузки:</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto custom-scrollbar p-2 bg-black/20 rounded-xl">
                        {Object.keys(exportHierarchy).sort().map(c => (
                            <label key={c} className="flex items-center gap-3 p-3 bg-gray-800/40 border border-gray-700 rounded-lg cursor-pointer hover:bg-gray-800 transition-colors">
                                <input type="checkbox" checked={selectedCountries.has(c)} onChange={() => {
                                    const next = new Set(selectedCountries);
                                    if (next.has(c)) next.delete(c); else next.add(c);
                                    setSelectedCountries(next);
                                }} className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-emerald-500 focus:ring-emerald-500/30" />
