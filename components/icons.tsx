...
interface IconProps {
    className?: string;
    // Fix: Added optional small property to IconProps to support smaller icon variants used throughout the app.
    small?: boolean;
}

// Fix: Updated LoaderIcon to support the small prop by adjusting its default size.
export const LoaderIcon: React.FC<IconProps> = ({ className, small }) => {
    const size = small ? "w-4 h-4" : "w-5 h-5";
    return (
        <div className={`border-2 border-current border-t-transparent rounded-full animate-spin ${className || size}`}></div>
    );
};

// Fix: Updated SVG icons to support the small prop by adjusting their default size via className logic.
export const SuccessIcon: React.FC<IconProps> = ({ className, small }) => {
    const size = small ? "w-4 h-4" : "w-5 h-5";
    return (
        <svg className={className || size} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
export const SortIcon: React.FC<IconProps> = ({ className, small }) => {
    ...
    return (
        <svg className={className || size} fill="currentColor" viewBox="0 0 20 20">
...
export const SortUpIcon: React.FC<IconProps> = ({ className, small }) => {
    const size = small ? "w-3 h-3" : "w-4 h-4";
    return (
        <svg className={`${className || size} text-accent`} fill="currentColor" viewBox="0 0 20 20">
...
export const PotentialIcon: React.FC<IconProps> = ({ className, small }) => {
    const size = small ? "w-4 h-4" : "w-5 h-5";
    return (
        <svg className={className || size} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
