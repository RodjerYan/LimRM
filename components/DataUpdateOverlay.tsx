...
const DataUpdateOverlay: React.FC<DataUpdateOverlayProps> = ({ jobStatus }) => {
    ...
    const getIcon = () => {
        switch (status) {
            case 'processing':
            case 'pending':
                return <LoaderIcon className="w-12 h-12 text-indigo-600" />;
            case 'completed':
                return <CheckIcon className="w-12 h-12 text-emerald-600" />;
            case 'error':
                return <ErrorIcon className="w-12 h-12 text-red-600" />;
            default:
                return null;
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/40 backdrop-blur-md animate-fade-in">
            <div className="w-full max-w-lg bg-white p-8 rounded-2xl shadow-2xl border border-gray-200 text-center">
                ...
                <p className="text-sm text-gray-600 mb-6 min-h-[40px]">{message}</p>
