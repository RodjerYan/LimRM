...
const ActiveClientsTable: React.FC<ActiveClientsTableProps> = ({ clients, onClientSelect, disabled }) => {
    ...
    const handleCopyToClipboard = () => {
        const tsv = [
            ['Наименование', 'Адрес', 'Город/Группа', 'Регион', 'РМ', 'Бренд', 'Канал продаж', 'Точность координат'].join('\t'),
            ...sortedData.map(row => [
                row.name, row.address, row.city, row.region, row.rm, row.brand, row.type, row.accuracy
            ].join('\t'))
        ].join('\n');
    ...
    if (disabled || clients.length === 0) {
        return (
             <div className="bg-card-bg/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-indigo-500/10 opacity-50">
                <h2 className="text-xl font-bold mb-4 text-white">Индивидуальные клиенты (АКБ)</h2>
                <div className="text-center py-10 text-gray-400"><p>Загрузите и обработайте файл, чтобы увидеть данные.</p></div>
            </div>
        );
    }
    
    const accuracyStyles: Record<MapPoint['accuracy'], string> = {
        'exact': 'text-success',
        'geocoded': 'text-purple-400',
        'approximate': 'text-warning',
        'region': 'text-danger',
    };
    
    const accuracyLabels: Record<MapPoint['accuracy'], string> = {
        'exact': 'Точные (ОКБ/Файл)',
        'geocoded': 'OSM (Кэш)',
        'approximate': 'Центр города',
        'region': 'Центр региона',
    };
    ...
    return (
        <div className={`bg-card-bg/70 backdrop-blur-sm rounded-2xl shadow-lg border border-indigo-500/10 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
            <div className="p-4 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-700">
                <h2 className="text-xl font-bold text-white whitespace-nowrap">Индивидуальные клиенты ({clients.length})</h2>
                ...
                <table className="w-full text-sm text-left text-gray-300">
                    <thead className="text-xs text-gray-400 uppercase bg-gray-900/70 sticky top-0 backdrop-blur-sm">
                        <tr>
                            ...
                            <SortableHeader sortKey="accuracy">Точность</SortableHeader>
                        </tr>
                    </thead>
                    <tbody>
                         {paginatedData.map((row) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className={`px-4 py-3 font-semibold ${accuracyStyles[row.accuracy] || 'text-gray-500'}`}>{accuracyLabels[row.accuracy] || 'Нет данных'}</td>
                                    </tr>
                                );
                            })}
                    </tbody>
