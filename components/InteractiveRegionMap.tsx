...
const InteractiveRegionMap: React.FC<InteractiveRegionMapProps> = ({ data, selectedRegions, potentialClients, activeClients, flyToClientKey, theme = 'dark', onEditClient }) => {
    ...
    useEffect(() => {
        const fetchGeoData = async () => {
            const CACHE_NAME = 'limkorm-geo-v4'; // Bump version for precise manual borders
            ...
            try {
                ...
                // 4. UKRAINE SOURCE (For Kherson only, since others are in Manual)
                // We specifically look for Kherson here since it wasn't in the manual JSON.
                if (ukraineData && ukraineData.features) {
                    const extraTerritoryMap: Record<string, string> = {
                        'Kherson Oblast': 'Херсонская область',
                        // Note: Donetsk, Luhansk, Zaporizhia, Crimea are already in MANUAL_BOUNDARIES
                    };

                    const extraFeatures = ukraineData.features
                        .filter((f: any) => extraTerritoryMap[f.properties.name])
                        .map((f: any) => {
                            f.properties.name = extraTerritoryMap[f.properties.name];
                            return f;
                        })
                        .filter((f: any) => !manualNames.has(f.properties.name)); // Ensure no overlap
