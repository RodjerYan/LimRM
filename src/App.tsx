...
const DashboardContent: React.FC<{ isBlur: boolean }> = ({ isBlur }) => {
    ...
    useEffect(() => {
        // Do not fetch if in blur mode (not logged in)
        if (isBlur) return;
    ...
    return (
        <div className={`bg-primary-dark min-h-screen text-text-main font-sans transition-all duration-300 ${isBlur ? 'blur-sm pointer-events-none select-none overflow-hidden h-screen' : ''}`}>
            <main className={`max-w-screen-2xl mx-auto space-y-6 p-4 lg:p-6 transition-all duration-300 ${isAnyModalOpen && !isBlur ? 'blur-sm pointer-events-none' : ''}`}>
