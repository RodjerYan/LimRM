...
const extendedRegionCenters: Record<string, string> = {
    ...
    // -- Новые города для Краснодарского края --
    ...
    "динской": "Краснодарский край",
    "южный": "Краснодарский край",
    "казанская": "Краснодарский край",

    // -- Новые города для Ростовской области --
    ...
    // -- Новые населенные пункты для Адыгеи --
    "дондуковская": "Республика Адыгея",
    "козет": "Республика Адыгея",
...
const KRASNODAR_KRAI_VARIANTS = ['краснодарский край', 'краснодаский край', 'кк'].map(v => v.toLowerCase());
...
export function parseRussianAddress(address: string): ParsedAddress {
    ...
    // === 1. Приоритетные регионы по аббревиатурам и вариантам ===
    if (MOSCOW_OBLAST_VARIANTS.some(v => lower.includes(v))) {
        region = 'Московская область';
    }

    if (!region && KRASNODAR_KRAI_VARIANTS.some(v => lower.includes(v))) {
