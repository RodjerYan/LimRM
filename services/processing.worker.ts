...
const parseDateValue = (val: any): number | null => {
    ...
    if (typeof val === 'number') {
        if (val > 30000 && val < 60000) return new Date((val - 25569) * 86400 * 1000).getTime();
        return null;
    }
    ...
    try {
        ...
        if (rawSheetData && rawSheetData.length > 0) {
            ...
            const jsonData = rows.map(rowArray => {
                const obj: any = {};
                headers.forEach((h, i) => { if (h) obj[h] = rowArray[i]; });
                return obj;
            });
...
async function processFile(jsonData: any[], headers: string[], { okbData, cacheData, postMessage }: CommonProcessArgs) {
    ...
    if (okbData) {
        okbData.forEach(row => {
            ...
            if (canonicalRegion && canonicalRegion !== 'Регион не определен') {
                ...
                if (!okbByRegion[canonicalRegion]) okbByRegion[canonicalRegion] = [];
    ...
    if (cacheData) {
        for (const rm of Object.keys(cacheData)) {
            for (const item of cacheData[rm]) {
                ...
                if (item.isDeleted) { deletedAddresses.add(normalizedTarget); continue; }
                if (!cacheAddressMap.has(normalizedTarget)) {
                    cacheAddressMap.set(normalizedTarget, { lat: item.lat, lon: item.lon, originalAddress: item.address, isInvalid: item.isInvalid, comment: item.comment });
                }
                if (item.history) {
                    ...
                    for (const entry of historyEntries) {
                        ...
                        if (normalizedOld && normalizedOld !== normalizedTarget) cacheRedirectMap.set(normalizedOld, normalizedTarget);
                    }
                }
            }
        }
    ...
    for (let i = 0; i < jsonData.length; i++) {
        ...
        if (i > 0 && i % 5000 === 0) postMessage({ type: 'progress', payload: { percentage: 10 + Math.round((i / jsonData.length) * 85), message: `Обработка: ${i.toLocaleString('ru-RU')}...` } });
        ...
        if (!rm) { unidentifiedRows.push({ rm: 'РМ не указан', rowData: row, originalIndex: i }); continue; }
        ...
        if (clientAddress) {
            ...
            if (cacheRedirectMap.has(normalizedRaw)) {
                ...
                if (targetEntry) { clientAddress = targetEntry.originalAddress || clientAddress; normalizedRaw = newNormalizedTarget; } 
                else { normalizedRaw = newNormalizedTarget; }
                if (deletedAddresses.has(normalizedRaw)) continue;
            }
        }
        ...
        if (cacheEntry && cacheEntry.isInvalid) { unidentifiedRows.push({ rm, rowData: row, originalIndex: i }); continue; }
        ...
        if (!isCityFound && !isRegionFound && !isCached) { unidentifiedRows.push({ rm, rowData: row, originalIndex: i }); continue; }
        ...
        if (!aggregatedData[key]) {
            aggregatedData[key] = { key, clientName: `${regionForAggregation} (${brand} - ${packaging})`, brand, packaging, rm, city: groupNameForAggregation, region: regionForAggregation, fact: 0, potential: 0, growthPotential: 0, growthPercentage: 0, clients: new Map<string, MapPoint>() };
        }
        ...
        if (!uniquePlottableClients.has(normalizedRaw)) {
            ...
            if (isCached && cacheEntry) {
                ...
                if (finalAddress && !newAddressesToCache[rm].some(item => item.address === finalAddress)) newAddressesToCache[rm].push({ address: finalAddress });

                const okbEntry = okbCoordIndex.get(normalizedRaw);
                if (okbEntry) { lat = okbEntry.lat; lon = okbEntry.lon; } 
                else if (finalAddress && !isCachedFlag) {
                    if (!addressesToGeocode[rm]) addressesToGeocode[rm] = [];
                    if (!addressesToGeocode[rm].includes(finalAddress)) addressesToGeocode[rm].push(finalAddress);
                }
            }
            uniquePlottableClients.set(normalizedRaw, { key: normalizedRaw, lat, lon, isCached: isCachedFlag, status: 'match', name: clientName, address: displayAddress, city: groupNameForAggregation, region: regionForAggregation, rm, brand, packaging, type: findValueInRow(row, ['канал продаж']), contacts: findValueInRow(row, ['контакты']), originalRow: row, fact: weight, comment });
        } else {
    ...
    for (const item of Object.values(aggregatedData)) {
        ...
        finalData.push({ ...item, potential, growthPotential: Math.max(0, potential - item.fact), growthPercentage: potential > 0 ? (Math.max(0, potential - item.fact) / potential) * 100 : 0, potentialClients: regionPotentialClients, clients: Array.from(item.clients.values()) });
    }

    postMessage({ type: 'result', payload: { aggregatedData: finalData, plottableActiveClients, unidentifiedRows, okbRegionCounts, dateRange } });
    ...
    if (newAddressRMs.length > 0) {
        ...
        for (const rmName of newAddressRMs) {
            try {
                ...
                await fetch('/api/data-service?action=add-to-cache', { 
                    method: 'POST', 
        ...
        for (const rmName of geocodeRMs) {
            ...
            for (let i = 0; i < addresses.length; i++) {
                ...
                for (let attempt = 1; attempt <= 3; attempt++) {
                    try {
                        const response = await fetch(`/api/data-service?action=geocode&address=${encodeURIComponent(address)}`);
                        if (response.ok) { coords = await response.json(); break; }
                    } catch (e) { }
...
async function processCsv(file: File, args: CommonProcessArgs) {
    ...
    const parsePromise = new Promise<{ data: any[], meta: ParseMeta }>((resolve, reject) => {
        PapaParse(file, {
            ...
            complete: (results: ParseResult<any>) => resolve({ data: results.data, meta: results.meta }),
    ...
    try {
        ...
        await processFile(jsonData, headers, args);
    } catch (error) { throw new Error(`Failed to parse CSV file: ${(error as Error).message}`); }
}
