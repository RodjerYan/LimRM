    ...
    const applyDeltasToData = useCallback((rows: AggregatedDataRow[], deltas: DeltaItem[]) => {
        ...
        sortedDeltas.forEach(currentDelta => {
            ...
            if (existingChange && existingChange.type !== 'delete') {
                ...
                // This ensures we keep coordinates from a previous move even if we later just update a comment.
                ...
                // If the current delta is a delete, it overrides everything.

                const finalType = currentDelta.type === 'delete' ? 'delete' : 'update';

                changesByKey.set(currentDelta.key, {

                    ...currentDelta, // Use the latest delta's metadata (timestamp)
                ...
                // First time seeing this key, or the previous state was a delete (which resets history).
        ...
        // This ensures the address map points to the fully merged, authoritative state.

        const changesByAddress = new Map<string, DeltaItem>();

        changesByKey.forEach(delta => {

            // Use the original key for address lookup, not an address from the payload.
        ...
        return rows.map(group => {
            ...
            const activeClients = group.clients.reduce((acc, client) => {
                ...
                if (relevantDelta.type === 'delete') {
                    ...
                    // Skip pushing to acc -> effectively deletes
                ...
                if (relevantDelta.type === 'update' && relevantDelta.payload) {
                    ...
                    // Apply the FULLY MERGED payload

                    // This guarantees we have the cumulative effect of all edits

                    acc.push({

                        ...client,

                        ...relevantDelta.payload,

                        // Ensure status is match if coords are present in the final merged payload

                        status: (relevantDelta.payload.lat && relevantDelta.payload.lon) ? 'match' : client.status
