...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // 1. ПРОВЕРЯЕМ АВТОРИЗАЦИЮ В НАЧАЛЕ ЗАПРОСА
    const cookies = parseCookies({ req });
    const storedTokens = cookies.google_tokens;

    if (!storedTokens) {
        // Если токенов нет — возвращаем ошибку 401 (Unauthorized)
        return res.status(401).json({ error: 'Auth required', needLogin: true });
    }

    try {
        // 2. ВОССТАНАВЛИВАЕМ ДОСТУП
        ...
        if (req.method === 'POST') {
            ...
            // Legacy Cache Operations (оставляем для совместимости, но они могут не работать без прав на таблицу)
            // При использовании OAuth пользователя, у пользователя должен быть доступ к этим таблицам
