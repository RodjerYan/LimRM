...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (config?.responseMimeType === 'application/json') {
            ...
            // FIX: Add robust check for response.text to prevent crashes on empty/filtered responses
            // and fix the Vercel build error `TS18048: 'response.text' is possibly 'undefined'`.
        ...
        // Default to streaming text request
